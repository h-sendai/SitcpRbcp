...
def main():
    ...
    data = rbcp.read_register_f('192.168.0.16', 0x10, '>B')[0] # [0]: read_rester_f returns struct.unpack tuple
    print '0x%02x' % (data)
...
class SitcpRbcp:
    ...
    def _send_recv_command_packet(self, command, ip_address, address, length, id, data = ''):
        ...
        if (command == 'READ'):
            ...
            raise ValueError('Unknown command in _send_recv_command_packet.  \
This is a bug of the SitcpRbcp module (not a bug of user program)')
        ...
        if command == 'WRITE':
            for i in range(0, length):
                if data[i] != reply_data[i]:
                    raise ValueError('original data and reply data does not match: orig: %02x, reply: %02x' \
                        % (data[i], reply_data[i]))
                if self.verify_on_write:
                    ...
                    for i in range (0, length):
                        if data[i] != re_read_data[i]:
                            raise ValueError('original data and reply data does not match: orig: 0x%02x, reply: 0x%02x' % (data[i], re_read_data[i]))
    ...
    def read_register_f(self, ip_address, address, format, id = 1):
        """read one register with format. 
           format is a format string of the struct package.
           Return read value as struct.unpack tuple.  If single value, use [0] as follows:
           Example: read_register_f('192.168.0.16', 0x01, '>B')[0]
        """
